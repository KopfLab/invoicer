...
create_invoice_je <- function(
    template, ref_no, equipment_id, invoice_date, purpose,
  ...
  stopifnot(
    "`template` required" = !missing(template),
    "`ref_no` required" = !missing(ref_no),
    "`equipment_id` required" = !missing(equipment_id),
  ...
  values <- list(
    ...
    expense_desc = items_description,
  ...
  # journal entry
  output_filepath <- create_invoice_je_output_path(ref_no, equipment_id, customer_name, customer_affiliation, total)
  fields <- get_journal_entry_fields()
  create_journal_entry(
    values = values, fields = fields, template_filepath = template, output_filepath = output_filepath
  )
...
create_invoice_external <- function(
    template, reference, ref_no, quote_nr, po_nr, equipment_id,
    invoice_date, purpose, st_to,
    customer_name, customer_affiliation, customer_address, customer_email,
    service_lab, store_url,
    invoice_contact, invoice_contact_email, invoice_contact_phone,
    items_description, items_quantity, items_unit_price, items_unit_size
) {
  ...
  stopifnot(
    ...
    "`ref_no` required" = !missing(ref_no),
    "`quote_nr` required" = !missing(quote_nr),
    "`equipment_id` required" = !missing(equipment_id),
    "`invoice_date` required" = !missing(invoice_date),
    "`purpose` required" = !missing(purpose),
    "`st_to` required" = !missing(st_to),
    ...
    "`customer_address` required" = !missing(customer_address),
    ...
    "`invoice_contact_phone` required" = !missing(invoice_contact_phone),
    "`items_description` required" = !missing(items_description),
    "`items_quantity` required" = !missing(items_quantity),
    "`items_unit_price` required" = !missing(items_unit_price),
    "`items_unit_size` required" = !missing(items_unit_size)
  ...
  values <- list(
    ...
    invoice_contact_phone = invoice_contact_phone,
  ...
  # journal entry
  output_filepath <- create_invoice_ext_output_path(ref_no, equipment_id, customer_name, customer_affiliation, total)
  create_invoice(
...
create_invoice_je_output_path <- function(ref_no, equipment_id, customer_name, customer_affiliation, amount) {
  sprintf("%s_%s_int_%s_%s_%sUSD.xlsx", ref_no, equipment_id,
          stringr::str_replace_all(customer_name, "[ ,/.]+", "_"),
          stringr::str_replace_all(customer_affiliation, "[ ,/.]+", "_"),
          paste(amount))
}

create_invoice_ext_output_path <- function(ref_no, equipment_id, customer_name, customer_affiliation, amount) {
  sprintf("%s_%s_ext_%s_%s_%sUSD.Rmd", ref_no, equipment_id,
          stringr::str_replace_all(customer_name, "[ ,/.]+", "_"),
          stringr::str_replace_all(customer_affiliation, "[ ,/.]+", "_"),
          paste(amount))
}
